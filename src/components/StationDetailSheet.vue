...
const props = defineProps<{
  ...
  /**
   * 'nearby' : 홈/반경 기반 — 현재 위치 → 주유소 경로를 emit으로 전달
   * 'route'  : 경로 추천 페이지 — routeStore 통해 내비 모드 전환 (기존 동작)
   */
  mode?: 'nearby' | 'route'
}>()
...
async function handleStartNavigation() {
  ...
  try {
    if (props.mode === 'nearby') {
      // 반경 기반: routeStore의 origin을 현재 위치로 세팅 후 경로만 계산
      // 현재 위치는 routeStore.currentLocation 또는 부모가 넘긴 center 사용
      routeStore.selectRecommendedStation(props.station)

      // origin이 없으면 currentLocation을 임시 origin으로 세팅
      if (!routeStore.origin && routeStore.currentLocation) {
        routeStore.setOrigin({
          name: '현재 위치',
          lat: routeStore.currentLocation.lat,
          lng: routeStore.currentLocation.lng,
        })
      }

      await routeStore.buildRouteToStation()

      // 계산된 경로를 부모(HomePage)에 전달해서 지도에 표시
      emit('route-ready', routeStore.routePath)
    } else {
      // 경로 추천 모드: 기존 내비 모드 전환
      routeStore.selectRecommendedStation(props.station)
...
.sheet-leave-to .detail-sheet { transform: translateY(100%); }
...
.price-box { border-radius: 18px; padding: 14px; }
...
.tag { padding: 6px 12px; border-radius: 999px; font-size: 12px; font-weight: 700; }
...
.action-btn.secondary { background: #f3f4f6; color: #111827; }
.action-btn.secondary.disabled { opacity: 0.4; pointer-events: none; }
